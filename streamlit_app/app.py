...
def create_pdf_report(pred_labels, accuracy, confidence, time_axis, y_ai, name, duration_hours, sampling_rate=100):
    ...
    # Explicitly define usable width (Page width - margins)
    ...
    pdf.cell(0, 8, safe_text(f"Average Confidence: {confidence:.2f}%"), ln=True)
    pdf.cell(0, 8, safe_text(f"Accuracy vs GT: {accuracy}"), ln=True)
    pdf.cell(0, 8, safe_text(f"Sleep Efficiency: {efficiency:.1f}%"), ln=True)
    
    pdf.set_y(-25)
    pdf.set_text_color(150, 150, 150)
    pdf.set_font("helvetica", "I", 9)
    # Changed 0 to effective_width to prevent line_break error
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        plt.step(time_axis, y_ai, where="post", color='#2980b9')
    ...
    for c in comments:
        # Crucial fix: use effective_width instead of 0
        pdf.multi_cell(effective_width, 8, safe_text(f"- {c}"))

    # Determine output format based on library version
